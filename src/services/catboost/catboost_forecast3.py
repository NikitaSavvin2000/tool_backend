...
def create_features(df: pd.DataFrame, time_column: str, col_target: str, lag_features: List[int]) -> pd.DataFrame:
    ...
    df_features['diff_168'] = df_features[col_target].diff(168)
...
def forecast_catboost_method(
    ...
    return pd.DataFrame({time_column: forecast_dates, col_target: predictions})
...
def forecast_catboost_method_diff(
    ...
    model = CatBoostRegressor(
        iterations=catboost_params.get('iterations', 2000),
        learning_rate=catboost_params.get('learning_rate', 0.03),
        depth=catboost_params.get('depth', 8),
    ...
    for step in range(forecast_steps):
        ...
        for lag in lag_features:
            ...
            if lag_col in temp.columns:
                fact_vals = df_extended[col_target].shift(lag)
                pred_vals = df_extended['__pred_delta'].shift(lag)
                temp[lag_col] = fact_vals.combine_first(pred_vals)
...
def lag_selection_catboost(
    ...
    # Ограничение лага, чтобы не выйти за границы
...
def user_predict_catboost(
    ...
    forecast_method: str = "level"  # "level" или "delta"
) -> dict:
    ...
    catboost_params = {
        'iterations': 100 if forecast_method == "level" else 2000,
        'learning_rate': 0.1 if forecast_method == "level" else 0.03,
        'depth': 6 if forecast_method == "level" else 8,
        'l2_leaf_reg': 1 if forecast_method == "level" else 3, 
        'random_seed': 42,
        'verbose': False,
        'loss_function': 'RMSE' if forecast_method == "level" else 'MAE'
    }
