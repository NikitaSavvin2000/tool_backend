...
from catboost import CatBoostRegressor  # заменено
...
model_architecture_params = [{
    "objective": "RMSE",  # CatBoost использует RMSE вместо reg:squarederror
    "task_type": "GPU",   # эквивалент device="cuda"
    "iterations": 1000,
    "learning_rate": 0.1,
    "depth": 15,
    "subsample": 0.9,
...
def forecast_XGBoost_sistem(
    ...
    cat_model = CatBoostRegressor(**model_params)  # заменено
    X_reshaped = X.reshape(X.shape[0], -1)
    cat_model.fit(X_reshaped, y, verbose=False)  # добавлено verbose=False для подавления вывода

    # Генерация прогнозов
    x_input = x_input.reshape((1, lag, n_features))
    predict_values = _make_xgboost_predictions(x_input, df_test.values, n_features, cat_model, lag)  # заменено имя модели
...
def _make_xgboost_predictions(
        ...
        df_test: np.ndarray,
        n_features: int,
        model: CatBoostRegressor,  # заменено
        lag: int
) -> List[float]:
    """
    Генерирует прогнозы для будущего горизонта с использованием обученной модели CatBoost.
...
def user_predict_XGBoost(
    ...
    """
    Генерирует прогноз временного ряда с использованием CatBoost.
