...
def forecast_N_method(
    ...
    """
    Реализация метода N с использованием CatBoostRegressor и walk-forward подхода.
    """
    ...
    # Разделение на обучающую и будущую части
    ...
    # Walk-forward по каждой точке будущего
    for i in range(len(df_future)):
        # Подготовка обучающих данных (ограничим историей, например, 1000 точек)
        start_idx = max(0, len(current_df) - 1000)
        window = current_df.iloc[start_idx:].copy()

        # Пропускаем, если недостаточно данных для лага
        if len(window) <= lag:
            # Используем последнее известное значение как fallback
            last_val = window[col_target].iloc[-1] if not window.empty else 0.0
            pred = last_val
        else:
        ...
        # Добавляем предсказание в current_df для следующего шага
        new_row = df_future.iloc[[i]].copy()
        new_row[col_target] = pred
        current_df = pd.concat([current_df, new_row], ignore_index=True)

    # Формируем df_real_predict (только целевая колонка и индекс)
