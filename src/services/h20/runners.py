...
import numpy as np
import pandas as pd
from src.services.h20.h20_forecast import user_predict_h20
import plotly.graph_objects as go
from src.utils.metrics import calculate_metrics


func_to_predict = user_predict_h20


home_path = os.getcwd()
data_path = os.path.join(home_path, "src", "data")
experiment_path = os.path.join(home_path, "src", "services", "h20",  "experiments")
...
if __name__ == "__main__":
    ...
    for data_name_to_experiment in collection_data:
        ...
        y_pred = y_pred[1:] # Делаем так тк внутри алгоритма намеренно добавляем в прогноз первую дату настоящее значение, чтобы график выглядел слитно а не разрывисто
